...
After that i will highlight the most relevant works in video object detection, like FGFA\cite{zhu2017flowguidedfeatureaggregationvideo} and MEGA\cite{chen2020memoryenhancedgloballocalaggregation}, as well as the two models used in this thesis, TransVOD++\cite{zhou2022transvod} and YOLOV++\cite{shi2024yolovpp}.
...
\section{Transvod++}
Transvod\cite{zhou2022transvod} is a end to end video object detection model base on DETR\cite{carion2020endtoendobjectdetectiontransformers}.
...
The paper that was published in 2022, was able to surpass previous state of the art video object detection models on the ImageNet VID\cite{russakovsky2015ImageNet} benchmark with a mAP (\%) of 85.5 on the validation set, while being still near real time capable with 22.7 FPS on a Nvidia TITAN RTX GPU.
...
\chapter{Discussion}
The aim of this this was to evaluate wheather video object detection models can leverage temporal context to improve robustness against perturbations in UAV-based videos.
...
For TransVOD++, sampling plays a crucial role for performance. The model was evaluated on three different sampling strategies: local sampling with a temporal stride of 1 frame, local sampling with a temporal stride of 8 frames, and global sampling.
...
To answer the main research question regarding robustness against perturbations, the Visdrone dataset was perturbated multible ways, simulating real-world corruptions that can occur in UAV-based videos.
...
Against the prehand expected outcome, using increasing the temporal context window does not show any significant improvement in robustness as seen in Table~\ref{tab:perturbated_results}.
...
Furthermore this shows that the models are not able to use information from multiple perturbated frames to compensate for the precision loss caused by the perturbations.
Therefore aggregating weak object features from multiple perturbated frames does not perform better than relying on a single clean frame.
...
Pixelation is the most challenging perturbation type of all perturbations, both model show a significant performance drop, however YOLOV++ detection pipeline completely fails.
Since pixelation causes the most infomration loss, it is expected that the models struggle to detect objects under this perturbation.
...
Since this research focused only only on image-level perturbations, future work could explore the impact of temporal perturbations, such as frame drops or temporal jitter and dynamic motion blur, which are common in UAV videos.
