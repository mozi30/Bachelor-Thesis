...
\begin{figure}
    \centering
    \includegraphics[width=0.9\textwidth]{graphics/transvod/model.png}
...
This baseline is used to extract spatial features from individual frames.
...
As it can be seend in Figure~\ref{fig:transvod++_architecture}, each image is first processed by the deformable DETR backbone to extract spatial features.
...
Finally, the selected object queries from the current frame and reference frames are aggregated using a temporal transformer to produce the final detection results.
...
\begin{axis}[
    ...
    ylabel={mAP},
    xtick={1,2,3,4},
    xticklabels={Clean,Low,Mid,High},
    ymin=0, ymax=22,
    grid=both,
...
% YOLOV++ single-frame
\addplot coordinates {(1,17.2) (2,14.5) (3,10.7) (4,4.8)};
\addlegendentry{YOLOV++ single-frame}

% YOLOV++ temporal
\addplot coordinates {(1,18.0) (2,15.7) (3,11.3) (4,5.0)};
\addlegendentry{YOLOV++ + temporal}

% TransVOD++ single-frame
\addplot coordinates {(1,19.4) (2,19.3) (3,18.0) (4,13.6)};
\addlegendentry{TransVOD++ single-frame}

% TransVOD++ temporal
\addplot coordinates {(1,20.4) (2,20.2) (3,18.8) (4,14.2)};
\addlegendentry{TransVOD++ + temporal}
...
\small
\begin{tabular}{p{0.58\linewidth}cccc}
\toprule
...
\caption{Robustness under image corruptions. Top: mAP vs severity. Bottom: mAP table (clean/low/mid/high).}
\label{fig:robustness_combo}
\end{figure}
